...
{
    public partial class ControlerSuspendedMedicalLeaveView : UserControl
    {
        string Operation => "SuspendareContract";
        ...
        public ControlerSuspendedMedicalLeaveView(IDbContextFactory<AppDbContext> db, ISessionContext session)
        {
            InitializeComponent();
            _dbFactory = db;
        ...
        private void ControlerSuspendedMedicalLeaveView_Load(object sender, EventArgs e)
        {
...
string authorIdStr)
        ...
        private void ControlerSuspendedContractsView_Load(object sender, EventArgs e)
        {
        ...
        private async Task LoadMedicalLeaveAsync()
        {
            await using var db = await _dbFactory.CreateDbContextAsync();

            var query = db.MedicalLeaveEntry
                .AsNoTracking()
                .Join(db.ContractsRu.AsNoTracking(),
                      ml => ml.ContractId,
                      ctr => ctr.IdContract,
                      (ml, ctr) => new { ml, ctr })
                .Join(db.Set<RegesSync>().AsNoTracking(),
                      x => x.ctr.PersonId,
                      rs => rs.PersonId,
                      (x, rs) => new { x.ml, x.ctr, rs })
                .Join(db.Set<RegesContractSync>().AsNoTracking(),
                      x => x.ctr.IdContract,
                      rc => rc.IdContract,
                      (x, rc) => new { x.ml, x.ctr, x.rs, rc })
                .Join(db.Set<Person>().AsNoTracking(),
                      x => x.ctr.PersonId,
                      per => per.PersonId,
                      (x, per) => new { x.ml, x.ctr, x.rs, x.rc, per })
                .Where(x => x.ctr.ContractStatusId == 1
                         && x.rs.RegesEmployeeId != null
                         && x.rc.RegesContractId != null)
                .Select(x => new
                {
                    ContractId = x.ctr.IdContract,
                    Name = ((x.per.LastName ?? "").Trim() + " " + (x.per.FirstName ?? "").Trim()).Trim(),
                    BeginDate = x.ml.StartDate,
                    EndDate = x.ml.EndDate
                });

            // optional: inspect generated SQL
        ...
        /// <summary>
        /// Wires up all DataGridView behaviors for dgvContracteSuspendate.
        /// </summary>
        /// <remarks>
