...
{
    public partial class ControlerCorrectionStopedSuspendedContractsView : UserControl
    {
        string Operation => "CorectieSuspendareContract";
...
#endif
        public ControlerCorrectionStopedSuspendedContractsView(IDbContextFactory<AppDbContext> db, ISessionContext session)
        {
            InitializeComponent();
            _dbFactory = db;
        ...
        private void ControlerCorrectionStopedSuspendedContractsView_Load(object sender, EventArgs e)
        {
        ...
        private void BuildBarsAndLayout()
        {
            ...
            lblTitle = new Label
            {
                Text = "Corectie contracte suspendate",
                AutoSize = true,
            ...
            {
                Text = "Trimitere corectie contracte suspendate",
                AutoSize = true,
        ...
        private async Task LoadContractsAsync()
        {
            ...
            var baseQuery =
                ...
                where rs.RegesEmployeeId != null
                   ...
                   && cs.RegesSyncVariable == 3 // 3 = contreactul a fost corectat
                orderby c.IdContract, cc.Updated_At descending
                select new { c.IdContract, cc.RegesContractId, cc.Updated_At }
                into x
                group x by x.IdContract into g
                orderby g.Key
                select g.OrderByDescending(z => z.Updated_At).First();

            System.Diagnostics.Debug.WriteLine(baseQuery.ToQueryString()); // <-- show SQL
            ...
            foreach (var it in baseRows)
            {
                ...
                DateTime? dataInceput = susp?.SuspensionStartDate
                    ?? await db.ContractRuSuspended.AsNoTracking()
